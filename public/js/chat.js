        ...
        $.ajax({
                ...
                success:function(data) {
                    ...
                    if(!isPhone)//pc
                        ...
                        document.getElementById("chatroom_user").innerHTML='<div id="chatroom_'+user_remote_id+'" style="position:absolute;bottom:20px;width:'+(document.body.clientWidth-90)+'px;left:58px"><div id="chatroom_scroll" class="thumbnail" style="background:#f2f2f2;border:0px;margin-bottom:0px;height:360px; overflow-y:auto;"><p style="text-align:center;font-size:14px;padding-top:10px;margin-bottom:0px;">chat history with '+user_remote_name+'</p><button onclick="toggleChat('+user_remote_id+')" class="btn btn-xs bambu-color1" style="position:absolute;top:0px;right:0px;z-index:1000">CLOSE</button><div id="dialog_chatmessage_'+user_remote_id+'" style="text-align:left;"></div></div><textarea class="thumbnail form-control-1" id="dialog_sendtext_'+user_remote_id+'" placeholder="reply here" onkeydown="enterToSubmit(this,event)" style="width:'+(document.body.clientWidth-90)+'px;margin-bottom:11px;"></textarea>      <input id="'+user_remote_id+'" style="position:absolute;width:60px;bottom:12px;right:0px;z-index:1000" onclick="onSubmit('+user_remote_id+')" class="btn btn-xs" value="SEND" />';
...
function chatroom(){
    ...
    $.ajax({
            ...
            success:function(data) {
                ...
                {   document.getElementById("chatroom").innerHTML+='<div id="chatroom_nav" style="z-index:-1;border:4px solid #7f8c8d;border-radius:10px;position:absolute;bottom:-10px;left:0px;width:'+(document.body.clientWidth-25)+'px;height:480px;background:#fff;"><h6 style="color:black;">Messages</h6><img src="/img/cross.png" onclick="closeChat()" style="cursor:pointer;position:absolute;top:0px;right:0px;z-index:1000"></div><div id="chatroom_left" style="background:#DDDDDD;width:55px;height:424px;border-radius:6px;position:absolute;bottom:-5px;left:4px"></div>';
                }
