...
function onSubmit(id){
    ...
    if(send_text)
    {
        ...
        document.getElementById("dialog_chatmessage_"+user_remote_id).innerHTML+="<div align='right' style='padding-top:20px;'><div style='position:relative;width:auto; display:inline-block !important;background:#A0D468;border-radius:5px;padding:0px 10px 0px 10px;'><p style='padding-top:5px;font-size:15px;color:black;max-width:180px;'>" + send_text + "</p><div style='position:absolute;top:5px;right:-16px;width:0;height:0;font-size:0;border:solid 8px;border-color:#f2f2f2 #f2f2f2 #f2f2f2 #A0D468;'></div></div><img style='width:36px;margin:5px 0px 0px 16px;' class='img-circle user_profile_"+user_id+" src="+local_user_profile+"'></div>";
        $.ajax({
                type:"get",
                ...
                success:function(data) {
                    ...
                    for(i = data.message.length - 16; i < data.message.length; i++)
                    {
                        if(i>=0)
                        {
                            ...
                            document.getElementById("dialog_chatmessage_"+user_remote_id).innerHTML+="<div align='right' style='padding-top:20px;'><div style='position:relative;width:auto; display:inline-block !important;margin-right:5px;background:#A0D468;border-radius:5px;padding:0px 10px 0px 10px;'><p style='padding-top:5px;font-size:15px;color:black;max-width:180px'>"+data.message[i].message + "</p><div style='position:absolute;top:5px;right:-16px;width:0;height:0;font-size:0;border:solid 8px;border-color:#f2f2f2 #f2f2f2 #f2f2f2 #A0D468  ;'></div></div><img style='width:36px;margin:5px 0px 0px 16px;' class='img-circle user_profile_"+data.message[i].user_id+"'></div>";
                            }
                        }
                    }
